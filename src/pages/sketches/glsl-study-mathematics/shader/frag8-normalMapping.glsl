// 3Dレンダリング：法線マッピング（形状表面の法線ベクトルにテクスチャを作用させ、擬似的に凹凸感をマッピングする）
...
void main(){
  ...
  vec3 lPos = vec3(0.,0.,0.); // 点光源の位置
  if (dot(ray, groundNormal) < 0.0){ // 交差判定
    vec3 hit = cPos - ray * groundHeight / dot(ray, groundNormal); // 交点
    groundNormal.zx += grad2(hit.zx); // ノイズの勾配を法線に加える
    groundNormal = normalize(groundNormal); // 勾配の大きさを１に正規化
