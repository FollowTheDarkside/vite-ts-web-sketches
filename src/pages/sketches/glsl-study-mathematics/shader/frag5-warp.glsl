...
float fbm21(vec2 p, float g){ // ２変数fBM(非整数ブラウン運動)
  float val = 0.0; // 値の初期値
...
float warp21(vec2 p, float g){ // 2変数ドメインわーピング
  float val = 0.0; // 値の初期値
  for (int i = 0; i < 4; i++){
    val = base21(p + g * val);
  }
...
void main(){
  ...
  pos = 10.0 * pos;
  float g = abs(mod(time, 10.0) - 5.0); // gを[0,5]区間上動かす
  gl_FragColor = vec4(vec3(warp21(pos, g)), opacity);
}